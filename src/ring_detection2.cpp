...
float color[4];
float discard_color = 0.5; //color probability that it gets discarded
...
void image_callback(const ImageConstPtr& data, const ImageConstPtr& depth_img){
  ...
  if (nmb_rings_detected > 0){
    ROS_INFO("Found %d candidates", nmb_rings_detected);
    ...
    for (int i = 0; i<candidates.size(); i+=2)
    {
      ...
      // ellipse(showIm, e1, (0, 0, 255), 2);
      // ellipse(showIm, e2, (0, 0, 255), 2);
      ...
      if (0 <= box.x  && 0 <= box.width && box.x + box.width <= bridge_img->image.cols && 0 <= box.y && 0 <= box.height && box.y + box.height <= bridge_img->image.rows){
        ...
        //ROS_INFO("red %f green %f blue %f black %f", color[0], color[1], color[2], color[3]);
        ...
        //}
        //else {
            //ROS_INFO("Discarded because of bad colour recognition.");
        //}
